...
function stepHit(curStep:Int)
{
    ...
    if (curStep == 160)
    {
        ...
        FlxTween.tween(boyfriend, {color: 0xFFFFFFFF}, 0.3);
        FlxTween.tween(dad, {color: 0xFFFFFFFF}, 0.3);
...
function callLyrics(input:String, duration:Float)
    ...
    function shakeLyrics(intensity:Float, duration:Float)
    {
        if (textField != null)
            PlayState.camAlt.shake(intensity, duration);
    }